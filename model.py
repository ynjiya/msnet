...
class MSNet(nn.Module):
    def __init__(self, num_classes=3, zero_head=False, embed_dim=96, win_size=7):
        ...
        self.swin_unet = MSNetSwinTransformer3D(img_size=(128, 128, 128),
                                            patch_size=(4, 4, 4),
                                            in_chans=4,
...
class MSNetSwinTransformer3D(nn.Module):
    r""" MSNet -- medical segmentation network based on U-Net architecture and 3D Swin Transformer blocks
    ...
    def __init__(self, pretrained=None,
        ...
        print(
            "MSNetSwinTransformer3D expand initial----depths:{};depths_decoder:{};drop_path_rate:{};num_classes:{};embed_dims:{};window:{}".format(
                depths,
...
class PatchEmbed3D(nn.Module):
    """ 3D Patch partitioning and Linear embedding
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, prev_v1, prev_k1, prev_q1, prev_v2, prev_k2, prev_q2):
        ...
        Args:
            x: Input feature, tensor size (B, C, D, H, W).
            prev_v1, prev_k1, prev_q1: value, key, query matrices generated for W-MSA in the SwinTransformerBlock3D of encoder of the corresponding level
            prev_v2, prev_k2, prev_q2: value, key, query matrices generated for SW-MSA in the SwinTransformerBlock3D of encoder of the corresponding level
        """
        # calculate attention mask for SW-MSA
        B, C, D, H, W = x.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
...
class PatchExpand_Up(nn.Module):
    """ Second and third Patch Expanding Layer"""
...
class FinalPatchExpand_X4(nn.Module):
    """ Finale (4th) Patch Expanding Layer"""
